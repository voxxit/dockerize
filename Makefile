...
dist: deps dist-clean
	...
	mkdir -p dist/alpine-linux/armhf && GOOS=linux GOARCH=arm GOARM=6 go build -ldflags "$(LDFLAGS)" -a -tags netgo -installsuffix netgo -o dist/alpine-linux/armhf/dockerize
	...
	mkdir -p dist/linux/arm64 && GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o dist/linux/aarch64/dockerize
...
release: dist
	...
	tar -cvzf dockerize-alpine-linux-armhf-$(TAG).tar.gz -C dist/alpine-linux/armhf dockerize
